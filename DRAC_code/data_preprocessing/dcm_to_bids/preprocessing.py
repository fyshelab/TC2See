...
def convert_dicom(folder, tmp_folder, regex_runs, regex_anat, script_dir, throw_away_trs=5):
	...
	for dcm_filename in Path(folder).rglob("**/*.dcm"):
		...
		if sys.platform.startswith('linux') or sys.platform.startswith('darwin'):
			...
			os.system(script_dir / f"dcm2niix.exe" + f' -f "%i_%p" -p y -z y -o "{target}" "{folder.absolute()}"')
