...
with open(james_data_dir / "roi_names.json", 'r') as f:
    roi_names = json.load(f)

ROIs = {roi_names[str(i)]: [i] for i in range(1, 181)}


for subject_str in subject_strs:
    try:
        representations_path = james_data_dir / f"processed/glm_roi_representations/sub_{subject_str}"

        if  representations_path.exists(): ##### not
            representations_path.mkdir(parents=True, exist_ok=True)
        
            betas_file_path = f'{shared_dataset_root}/tc2see-betas_GLMsingle_T1w.hdf5'
            ...
            betas_std_safe = np.where(betas_std == 0, np.nan, betas_std)
            betas_normalized = (betas - betas_mean[:, None]) / betas_std_safe[:, None]  
            fmri_mask = h5_file[f'{subject}/fmri_mask'][()] # (67, 80, 68)
            h5_file.close()

            # Load the ROI mask
            roi_mask_file = f'{shared_dataset_root}/fmri_prep_vols_v2/derivatives/{subject}/{subject}_HCP_MMP1_dilated2.nii.gz'
            ...
            for ROI, ROI_mask in ROI_masks.items():
                roi_path = representations_path / ROI

                roi_path.mkdir(parents=True, exist_ok=True)
                glm_file_name = roi_path / f"reps_for_{ROI}.parquet"

                if numeric_glm_df.isna().any().any():
                    print(f"[NaN DETECTED in  numeric_glm_df] in ROI {ROI} for subject {subject_str}")

                # glm_df_tmp = numeric_glm_df.loc[:, ROI_mask].copy()
                roi_voxel_indices = [str(i) for i, keep in enumerate(ROI_mask) if keep and str(i) in numeric_glm_df.columns]
                glm_df_tmp = numeric_glm_df[roi_voxel_indices].copy()

                if glm_df_tmp.isna().any().any():
                    print(f"[NaN DETECTED in glm_df_tmp] in ROI {ROI} for subject {subject_str}")
        ...
        print(f"Error processing subject {subject_str}:")
        traceback.print_exc()
        print("\n\n")
        continue