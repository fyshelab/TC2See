...
corr = 'gaussian'
data_dir = results_dir / f"adaptive_rsa/{corr}/{lower}_{upper}_{step}"
...
try:
    permutation_path = data_dir / f'{lower}_{upper}_{step}_{corr}_permutations_r.json'
...
threshold_results_path = results_dir / data_dir / f'{lower}_{upper}_{step}_{corr}_range_results.json'
...
plt.tight_layout()
plt.savefig(results_dir / data_dir / f'{lower}_{upper}_{step}_similarity_threshold_analysis.png', dpi=300)



####################################################
# Get p-values for different ROIs at each threshold
####################################################
...
####################################################################################
# Plot average r value for each thrshold and variance as shaded area around the line
####################################################################################

if permutation_data_available:
    rois_to_plot = ROIs  
    # rois_to_plot = ["V1", "V2", "V3", "V4"]  
    
    plt.figure(figsize=(12, 8))
    
    colors = plt.cm.tab10(np.linspace(0, 1, len(rois_to_plot)))  # Generate distinct colors
    ...
    for i, roi in enumerate(rois_to_plot):
        ...
        for threshold in thresholds_to_permute:
            if (threshold in permutation_results.get(roi, {}) and 
                threshold in threshold_results and 
                roi in threshold_results[threshold]):
                
                thresholds_with_data.append(int(threshold))  # Convert to int for plotting
                
                # Get null distribution for this ROI and threshold
                null_distribution = np.array(permutation_results[roi][threshold])
                
                # Calculate mean and std of null distribution
                mean_r = np.mean(null_distribution)
                std_r = np.std(null_distribution)
                
                mean_correlations.append(mean_r)
                std_correlations.append(std_r)
        
        if thresholds_with_data:
            thresholds_array = np.array(thresholds_with_data)
            mean_array = np.array(mean_correlations)
            std_array = np.array(std_correlations)
            
            # Plot the mean line
            plt.plot(thresholds_array, mean_array, '-', 
                    color=colors[i], label=f'{roi}', linewidth=2)
            
            # Add shaded area for variance (mean ± std)
            plt.fill_between(thresholds_array, 
                           mean_array - std_array, 
                           mean_array + std_array,
                           color=colors[i], alpha=0.2)
    
    plt.xlabel('Expertise Similarity Threshold (%)')
    plt.ylabel('Pearson Correlation (Null Distribution)')
    plt.title('Null Distribution Mean ± SD for Each ROI Across Thresholds')
    plt.xticks(range(lower, upper + 1))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(results_dir / data_dir / f'{lower}_{upper}_{step}_null_distribution_analysis.png', 
            ...
            if not np.isnan(p_matrix[i, j]):
                ...
                if p_val > 0.05:
                    ax1.add_patch(plt.Rectangle((j - 0.5, i - 0.5), 1, 1,
                                                color='red', alpha=0.6))
    plt.tight_layout()
    plt.savefig(results_dir / data_dir / f'{lower}_{upper}_{step}_pvalue_heatmap.png', 
                dpi=300, bbox_inches='tight')
        ...
        for j in range(len(thresholds_to_permute)):
            if not np.isnan(r_matrix[i, j]):
                ax2.text(j, i, f'{r_matrix[i, j]:.2f}', ha='center', va='center', 
                        ...
                        fontsize=10)
    plt.tight_layout()
    plt.savefig(results_dir / data_dir / f'{lower}_{upper}_{step}_correlation_heatmap.png', 
                dpi=300, bbox_inches='tight')