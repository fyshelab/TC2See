...
permutation_path = results_dir / 'adaptive_rsa/permutation_test_nulls.json'
...
for threshold in thresholds_to_permute:
    for roi in ROIs:
        observed_r = threshold_results[str(threshold)][roi]['pearson_r']
        null_distribution = permutation_results[roi][str(threshold)]
...
plt.tight_layout()
plt.savefig(results_dir / 'adaptive_rsa/permutation_pvalues_plot.png', dpi=300, bbox_inches='tight')
...
for i, roi in enumerate(ROIs):
    for j, threshold in enumerate(thresholds_to_permute):
        if p_values[roi][threshold] is not None:
            ...
            r_matrix[i, j] = threshold_results[str(threshold)][roi]['pearson_r']


fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))


# P-value heatmap with red overlay for non-significant cells
...
plt.tight_layout()
plt.savefig(results_dir / 'adaptive_rsa/heatmap_analysis.png', dpi=300, bbox_inches='tight')